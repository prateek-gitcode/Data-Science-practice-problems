library(readr)
PlasticSales <- read_csv(file.choose())
view(PlasticSales)
...
#input t
PlasticSales_data["t"] <-cbind(1:60) 
...
#Partitioning 
train_set <- PlasticSales_data[1:40,]
test_set <- PlasticSales_data[40:20,]
...
sea_add_model <- lm(Sales ~ Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov, data = train_set)
...
Add_sea_Quad_model <- lm(Sales ~ t+t_square+Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov, data = train_set)
...
multi_sea_model <- lm(log_Sales ~ Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov, data = train_set)
...
A1 <- arima(Add_sea_Quad_model_final$residuals,order=c(5,0,0))
